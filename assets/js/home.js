...
const addTabContent = ($CurrentTabBtn, $currentTabPanel) =>{
    ...
    fetchData([['mealType', $CurrentTabBtn.textContent.trim().toLowerCase()], ...cardQueries], function (data){
        ...
        for(let i=0; i<12 ; i++){
            const{
                recipe:{
                    image, label: title, totalTime: cookingTime, uri },
            } = data.hits[i];
...
for(const [index, $sliderSection] of $sliderSections.entries()) {
    ...
    const /**{NodeElement} */ $silderWrapper = $sliderSection.querySelector("[data-slider-wrapper]");
    fetchData([...cardQueries, ["cuisineType", cuisineType[index]]], function (data){

        $silderWrapper.innerHTML = "";
        ...
        data.hits.map(item =>{
            ...
            $silderWrapper.appendChild($sliderItem);
